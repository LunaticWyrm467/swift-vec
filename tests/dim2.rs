...
fn vec2_angle_to() {
    ...
    assert!(up.angle_to(down).approx_eq(f32consts::PI));
    assert!(up.angle_to(left).approx_eq(-f32consts::FRAC_PI_2));
    assert!(up.angle_to(right).approx_eq(f32consts::FRAC_PI_2));
}
...
fn vec2_rotate() {
    ...
    assert!(rotated_90.angle().to_degrees().approx_eq(90f64));
}
...
fn vec2_interpolation() {
    ...
    let pre_start:     Vec2<f32> = -5.0.vec2();
    let post_terminal: Vec2<f32> =  3.0.vec2();
...
fn vec2_global() {
    ...
    let vec_i32:   Vec2<i32>   = 1.vec2();
    let vec_u32:   Vec2<u32>   = 2.vec2();
    let vec_usize: Vec2<usize> = 3.vec2();
    let vec_f64:   Vec2<f64>   = 4.0.vec2();
